...
import { adCopierRequestSchema, type AdCopierRequest } from "@shared/schema";
...
export function AdCopierForm({ onSuccess }: AdCopierFormProps) {
  ...
  const form = useForm<AdCopierRequest>({
    resolver: zodResolver(adCopierRequestSchema),
    mode: "onChange",
    defaultValues: {
      headline: "",
      primaryText: "",
  ...
  const onSubmit = async (data: AdCopierRequest) => {
    ...
    setLoading(true);
    try {
      const res = await apiRequest("POST", "/api/ad-copier/replicate", data);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="text-center mb-10">
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-violet-500/10 text-violet-600 dark:text-violet-400 text-xs font-semibold uppercase tracking-wider mb-5">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            <Button
              ...
              className="w-full btn-primary-glow bg-violet-600 hover:bg-violet-700 text-white gap-2"
            >
              {loading ? (
