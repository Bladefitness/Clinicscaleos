...
export function CreativeCard({ creative, index, imageUrl, builtPrompt, isGeneratingImage, onGenerateImage, onImprove, isImproving, animateModel = "kling", onSaveEditedImage, editorLayers }: CreativeCardProps) {
  ...
  useEffect(() => {
    ...
    apiRequest("POST", "/api/build-image-prompt", {
  ...
  const handleAnimate = async () => {
    ...
    try {
      ...
      const res = await apiRequest("POST", "/api/tools/animate-image", body);
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 flex-1 flex flex-col">
        ...
        {showPrompt && (
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-md px-3 py-2 mb-3 text-xs text-muted-foreground leading-relaxed whitespace-pre-wrap">
            {fetchingPrompt ? (
              ...
              <>
                ...
                {(builtPrompt || fetchedPrompt) && (
                  <p className="mt-2 text-[10px] text-emerald-600 dark:text-emerald-400">Export-ready: Use this prompt in Lovart, Midjourney, or other platforms.</p>
                )}
              </>
            )}
          </div>
