...
`;
  ...
  const handleGetImprovement = useCallback(async () => {
    ...
    try {
      const res = await apiRequest("POST", "/api/tools/improve-creative", {
        headline: creative.headline,
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <PipelineView
        ...
        imageUrls={imageUrls}
      />
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 p-4 rounded-xl card-premium border border-primary/20">
        ...
        <Link href="/campaign-hq">
          <Button variant="outline" size="sm" className="gap-1.5 text-primary border-primary/30 hover:bg-primary/10 hover:border-primary/50">
      ...
      {(research || (avatarList && avatarList.length > 0)) && (
        <Card className="mb-6 overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between gap-2 p-4 text-left hover:bg-muted/50 transition-colors rounded-lg"
          >
            <div className="flex items-center gap-2">
          ...
          {showWhy && (
            <div className="px-4 pb-4 pt-0 space-y-3 border-t border-border">
              {research?.summary && (
                <p className="text-sm text-muted-foreground pt-3">{research.summary}</p>
              )}
              {avatarList && avatarList.length > 0 && (
                <div className="text-sm">
                  <p className="font-medium text-foreground mb-2">Selected avatars for your offer:</p>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {improveStep === "feedback" ? (
            <div className="space-y-4">
              <div>
                <Label htmlFor="improve-feedback" className="text-sm text-muted-foreground">
              ...
              {improvedResult && (
                <div className="space-y-4">
                  {improvedResult.changes_summary && (
                    <p className="text-sm text-muted-foreground">{improvedResult.changes_summary}</p>
                  )}
                  <div>
