  ...
  ({ data, className, printMode = false }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "mx-auto max-w-[680px] rounded-xl border bg-card text-card-foreground shadow-lg",
          printMode && "border-slate-200 bg-white text-slate-900 print:shadow-none print:border print:rounded-lg",
          className
        )}
      >
        {/* Header */}
        <div
          className={cn(
            ...
            printMode ? "border-slate-200" : "border-border"
          )}
        >
          <div className="flex items-center gap-3">
            <div
              className={cn(
                ...
                printMode ? "bg-slate-100 text-slate-700" : "bg-primary/10 text-primary"
              )}
            >
          ...
          <div className="text-right">
            <p
              className={cn(
                ...
                printMode ? "text-slate-600" : "text-primary"
              )}
            >
        ...
        <div
          className={cn(
            ...
            printMode ? "border-slate-200 bg-slate-50/50" : "border-border bg-muted/30"
          )}
        >
          ...
          {data.status && (
            <span
              className={cn(
                ...
                data.status === "paid" && (printMode ? "bg-slate-200 text-slate-800" : "bg-success/10 text-success"),
                data.status === "pending" && (printMode ? "bg-amber-100 text-amber-800" : "bg-amber-500/10 text-amber-600 dark:text-amber-400"),
                data.status === "overdue" && (printMode ? "bg-red-100 text-red-800" : "bg-destructive/10 text-destructive")
              )}
            >
              {data.status}
            </span>
            ...
            <thead>
              <tr
                className={cn(
                  ...
                  printMode ? "border-slate-200 bg-slate-50" : "border-border bg-muted/20"
                )}
              >
            ...
            <tbody>
              {data.lineItems.map((item, i) => (
                <tr
                  ...
                  className={cn(
                    "border-b",
                    printMode ? "border-slate-100" : "border-border"
                  )}
                >
        ...
        <div className="flex justify-end px-6 py-5">
          <div className="w-full max-w-[280px] space-y-2">
            ...
            >
              <span>Total</span>
              <span className={printMode ? "text-slate-900" : "text-primary"}>
        ...
        {data.notes && (
          <div
            className={cn(
              "border-t px-6 py-4",
              printMode ? "border-slate-200 bg-slate-50/30" : "border-border bg-muted/20"
            )}
          >
