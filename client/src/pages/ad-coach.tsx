    ...
    <div className="flex-1 p-6 lg:p-10 overflow-auto">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-1 mb-6 bg-slate-100 dark:bg-slate-800 p-1 rounded-md w-fit flex-wrap">
          {tabs.map((tab) => (
            <Button
              ...
              className={`toggle-elevate ${activeTab === tab.id ? "toggle-elevated shadow-sm" : ""}`}
          ...
          <div className="space-y-6">
            <Card className="p-6">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <h2 className="text-lg font-bold text-foreground">Today's Pulse</h2>
                <AlertBadge level={pulse.alert_level} />
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed mb-4" data-testid="text-pulse-summary">{pulse.summary}</p>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{pulse.spend_today}</p>
                  <p className="text-xs text-muted-foreground">Spend Today</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{pulse.leads_today}</p>
                  <p className="text-xs text-muted-foreground">Leads Today</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{pulse.avg_cpl}</p>
                ...
                <div className="space-y-3">
                  {(pulse.losers || []).map((l: any, i: number) => (
                    <div key={i} className="border-b border-border last:border-0 pb-3 last:pb-0">
                      ...
                      <p className="text-xs text-red-600 dark:text-red-400 font-mono">{l.metric}</p>
                      <p className="text-xs text-muted-foreground mt-1">{l.insight}</p>
                    </div>
                  ))}
                </div>
              </Card>
        ...
        {activeTab === "weekly" && !weeklyLoading && weekly && (
          <div className="space-y-6">
            <Card className="p-6">
              <h2 className="text-lg font-bold text-foreground mb-3">Weekly Strategy Brief</h2>
              <p className="text-sm text-muted-foreground leading-relaxed mb-4" data-testid="text-weekly-summary">{weekly.executive_summary}</p>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{weekly.total_spend}</p>
                  <p className="text-xs text-muted-foreground">Total Spend</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{weekly.total_leads}</p>
                  <p className="text-xs text-muted-foreground">Total Leads</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-foreground">{weekly.avg_cpl}</p>
                  <p className="text-xs text-muted-foreground">Avg CPL</p>
                </div>
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="p-5">
                <h3 className="text-sm font-semibold text-red-600 dark:text-red-400 mb-2">Worst Performing</h3>
                <p className="text-sm font-medium text-foreground">{weekly.worst_performing?.name}</p>
                <p className="text-xs text-red-600 dark:text-red-400 font-mono">{weekly.worst_performing?.cpl} CPL | {weekly.worst_performing?.leads} leads</p>
                <p className="text-xs text-muted-foreground mt-1">{weekly.worst_performing?.insight}</p>
              </Card>
            </div>
            ...
            <Card className="p-5">
              <button onClick={() => setShowWeeklyDetails(!showWeeklyDetails)} className="w-full flex items-center justify-between" data-testid="button-toggle-weekly-details">
                <h3 className="text-sm font-semibold text-foreground">Trends & Recommendations</h3>
                {showWeeklyDetails ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
              </button>
              {showWeeklyDetails && (
                <div className="mt-4 space-y-4">
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Trends</p>
                    <div className="space-y-2">
                      {(weekly.trends || []).map((t: any, i: number) => (
                        <div key={i} className="flex items-center gap-3">
                          <TrendIcon direction={t.direction} />
                          <span className="text-sm text-foreground">{t.metric}:</span>
                          <span className="text-xs text-muted-foreground">{t.insight}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {activeTab === "chat" && (
          <Card className="flex flex-col" style={{ height: "calc(100vh - 250px)", minHeight: "400px" }}>
