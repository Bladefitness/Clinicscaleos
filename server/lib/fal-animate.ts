...
/** Normalize image input to URL or data URI for fal (they accept both). */
...
/** Build request body for each fal model. */
  ...
  switch (model) {
    ...
    case "kling3":
      return {
        prompt,
        image_url: imageInput,
        duration: String(Math.min(15, Math.max(3, durationSec))),
      };
    case "veo2":
      return {
        prompt,
        image_url: imageInput,
        aspect_ratio: "auto",
        duration: durationSec <= 6 ? "5s" : durationSec <= 7 ? "6s" : durationSec <= 8 ? "7s" : "8s",
      };
    default:
...
/** Animate image via fal.ai. Uses FAL_KEY. */
export async function animateImageWithFal(options: FalAnimateOptions): Promise<FalAnimateResult> {
  const key = getApiKey();
  if (!key) throw new Error("FAL_KEY not set. Add it to .env for image-to-video.");
